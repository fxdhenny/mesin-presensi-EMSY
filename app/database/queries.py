...
# Tambahkan ini di bagian paling bawah berkas app/database/queries.py Anda

# =====================================================================
# 5. FUNGSI AUTO-BACKUP SEBELUM OVERWRITE (Mendukung Presensi & Logs)
# =====================================================================
def ekspor_backup_otomatis_sebelum_ditimpa(db_path, target_dir):
    try:
        if not target_dir or not os.path.exists(target_dir):
            return False, "Flashdisk tidak terdeteksi!"
            
        # =====================================================================
        # PERUBAHAN DI SINI: Mengubah format menjadi Hari-Bulan-Tahun_Jam-Menit-Detik
        # Contoh Hasil: 01-06-2026_23-00-01
        # =====================================================================
        ...
        pesan_sukses = (
            f"Auto-Backup Berhasil dibuat di Flashdisk!\n\n"
            f"Arsip Berkas Amandemen:\n"
            f"- {len(data_harian)} baris data presensi harian\n"
            f"- {len(data_log)} baris data log riwayat audit"
        )
        return True, pesan_sukses

    except Exception as e:
        return False, f"Gagal mengamankan arsip otomatis: {str(e)}"


# =====================================================================
# 6. FUNGSI WIPE DATA LAMA (Tetap sama, memastikan pembersihan total)
# =====================================================================
def bersihkan_seluruh_data_lama(db_path):
    try:
        conn = sqlite3.connect(db_path)
        cursor = conn.cursor()
        
        # Menghapus bertahap sesuai hierarki Foreign Key agar tidak mengunci
        cursor.execute("DELETE FROM presensi_logs")
        cursor.execute("DELETE FROM presensi_harian")
